...
public class CopyOfGraphDelta {
    ...
    private final Map<PointsToGraphNode, Set<OrderedPair<PointsToGraphNode, TypeFilter>>> alreadyVisited;

    private boolean performedDomainClosure = false;

    public CopyOfGraphDelta(PointsToGraph g) {
        this.g = g;
        newBase = new LinkedHashMap<>();
        newIsSupersetOfRelations = new LinkedHashMap<>();
        alreadyVisited = new LinkedHashMap<>();
    }
    ...
    public Iterator<PointsToGraphNode> domainIterator() {
        ...
        return new PointsToGraph.ComposedIterators<>(newBase.keySet()
                                                            .iterator(),
    ...
    private void addSuperSetForChildrenOf(PointsToGraphNode b,
            ...
            Set<OrderedPair<PointsToGraphNode, TypeFilter>> visited,
            Stack<PointsToGraphNode> visitedFromBase) {
        ...
        while (iter.hasNext()) {
            ...
            Set<OrderedPair<PointsToGraphNode, TypeFilter>> alreadyVisited =
                    this.alreadyVisited.get(child.fst());
            if (alreadyVisited == null) {
                alreadyVisited = new HashSet<>();
                this.alreadyVisited.put(child.fst(), alreadyVisited);
            }
            for (PointsToGraphNode avb : visitedFromBase) {
                alreadyVisited.add(new OrderedPair<>(avb, (TypeFilter) null));
            }

            // now handle the recursive case, but only if the points to set is nonempty.
            if (new DeltaPointsToIterator(child.fst()).hasNext()) {
                visitedFromBase.push(b);
                addSuperSetForChildrenOf(child.fst(),
                                         newSet,
                                         visited,
                                         visitedFromBase);
                visitedFromBase.pop();
     ...
     * @return
     */
    public CopyOfGraphDelta combine(CopyOfGraphDelta d) {
        if (d != null) {
    ...
    public class DeltaPointsToIterator implements Iterator<InstanceKey> {
        ...
        public DeltaPointsToIterator(PointsToGraphNode n) {
            ...
            Set<OrderedPair<PointsToGraphNode, TypeFilter>> avb =
                    CopyOfGraphDelta.this.alreadyVisited.get(n);
            if (avb != null) {
                alreadyVisited.addAll(avb);
            }
