...
public class ApproximateCallSitesAndFieldAssignments {
    ...
    public Set<Approximation> findAllApproximations() {
        if (DO_NOT_RUN) {
        ...
        while (!q.isEmpty()) {
            ...
            if (pp instanceof CallSiteProgramPoint) {
                ...
                if (!isApproximate(currentInt)) {
                    ...
                    if (callees.isEmpty()) {
                        ...
                        approx.add(Approximation.getCallSiteApproximation(currentInt));
                        if (PRINT_APPROXIMATIONS) {
                            System.err.println("APPROXIMATING " + PrettyPrinter.methodString(cspp.getCallee())
                                    + " from " + PrettyPrinter.methodString(cspp.getCaller()) + " in "
                                    + current.getContext());
                        }
                    }
            ...
            if (g.getRegistrar().getStmtAtPP(pp) instanceof LocalToFieldStatement) {
                ...
                if (!killed.fst() && !isApproximateKillSet(stmt, current.getContext())) {
                    ...
                    approx.add(Approximation.getFieldAssignApproximation(new OrderedPair<>(stmt, current.getContext())));
                }
            }
